...
import { faCheckCircle, faTimesCircle, faMinusCircle, faFlag } from '@fortawesome/free-solid-svg-icons';
...
const QuizResultsPage = (props) => {
    ...
    // used in cases where the last question page was longer than screen height
    // then going to results page would preserve old scroll and might cut off the beginning of the page
    useEffect(() => {
        window.scrollTo(0, 0);
    }, []);
    ...
    const reportQuestion = async () => {
        ...
        try {
            const res = await fetch('/api/quiz/' + quiz.id + '/report/' + questionId, {
                method: 'POST'
            });
        ...
        return quiz.questions.map((question, index) => {
            const questionAnswers = question.answers.map((answer) => {
                const percentOfTotal = (
                    100 * answer.times_picked / quiz.times_finished
                ).toFixed(1);
    ...
    return (
        <div className="mx-auto mt-3 col-10 col-lg-9">
            ...
            <header className="text-center">
                ...
                <h3>Results</h3>
                <small>
                    *% shows how many times an answer was picked compared to all answers to the question
                </small>
            </header>
            <div className="my-4">
                {questions}
            ...
            <div className="d-flex justify-content-end">
                <Link to={'/'} className="btn btn-primary rounded-pill px-5 mb-3">To Home Page</Link>
            </div>
        </div>
    );
};
