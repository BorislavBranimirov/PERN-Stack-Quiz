...
router.route('/')
    .get(async (req, res) => {
        ...
        try {
            ...
            return res.status(500).json({ err: 'An error occurred while searching for quizzes' });
        }
    })
    .post(async (req, res) => {
        // create a new quiz
        ...
        */
        ...
        if (title.length > 150) {
            return res.status(422).json({ err: 'Quiz title length is limited to 150 characters' });
        }

        if (description) {
            if (typeof description !== 'string') {
                return res.status(422).json({ err: 'Quiz description must be a string' });
            } else {
                if (description.length > 200) {
                    return res.status(422).json({ err: 'Quiz description length is limited to 200 characters' });
                }
            }
        }
        ...
        if (image) {
            try {
                ...
                if (err.http_code === 404) {
                    ...
                    return res.status(500).json({ err: 'An error occurred while searching for the image' });
                }
            }
        }
        ...
        for (let i = 0; i < questions.length; i++) {
            if (!questions[i].text || typeof questions[i].text !== 'string') {
                return res.status(422).json({ err: `Question ${i + 1} has no text body provided` });
            }
            ...
            for (let j = 0; j < questions[i].answers.length; j++) {
                ...
                if (typeof answer.is_correct !== 'boolean') {
                    return res.status(422).json({
                        err: `Question ${i + 1}, answer ${j + 1} has no correct status provided`
                    });
                }
            ...
            if (correctCounter !== 1) {
                return res.status(422).json({
                    err: `All questions must have one and only one correct answer, please fix question ${i + 1}`
                });
            }
        }
        ...
        try {
            ...
            for (let i = 0; i < qs.length; i++) {
                ...
                for (let j = 0; j < qs[i].answers.length; j++) {
                    answersValuesText.push(`($${counter}, $${counter + 1}, $${counter + 2}, $${counter + 3})`);
                    answersValues.push(qs[i].answers[j].text, qs[i].answers[j].is_correct, j + 1, questionRow.id);
                    counter += 4;
                }
            ...
            client.release();
            return res.status(500).json({ err: 'An error occurred while creating quiz' });
        }
    });
...
router.route('/search')
    .get(async (req, res) => {
        ...
        try {
            ...
            const queryValues = [searchPattern, limitCount + 1, (page - 1) * limitCount];
...
router.route('/:id')
    .get(async (req, res) => {
        ...
        try {
            ...
            return res.status(500).json({ err: 'An error occurred while searching for quiz' });
        }
    })
    .delete(async (req, res) => {
        ...
        try {
            const quiz = (await db.query(
                'SELECT id, image FROM quizzes WHERE id = $1',
                [req.params.id]
            )).rows[0];
            ...
            if (quiz.image) {
                ...
                if (imageExists) {
                    const imageIsUsedByOtherQuizzes = (await db.query(
            ...
            const deletedQuiz = (await db.query(
            ...
            return res.status(500).json({ err: 'An error occurred while deleting quiz' });
        }
    });

router.route('/:id/report/:questionId')
    .post(async (req, res) => {
        ...
        try {
            ...
            return res.status(500).json({ err: 'An error occurred while reporting quiz question' });
        }
    });
...
router.route('/:id/submit')
    .post(async (req, res) => {
        ...
        try {
            ...
            quiz.questions = (await client.query(
            ...
            if (quiz.questions.length !== userAnswers.length) {
                await client.query('ROLLBACK');
                client.release();
                return res.status(422).json({ err: 'Incorrect number of answers provided' });
            }

            for (let i = 0; i < quiz.questions.length; i++) {
                quiz.questions[i].answers = (await client.query(
                ...
                if (userAnswerIndex === -1) {
                    await client.query('ROLLBACK');
                    client.release();
                    return res.status(422).json({ err: `Incorrect answer id provided for question ${i + 1}` });
                }

                // increment the answer picked counter
                quiz.questions[i].answers[userAnswerIndex] = (await client.query(
                    'UPDATE answers ' +
            ...
            await client.query('ROLLBACK');
            client.release();
            return res.status(500).json({ err: 'An error occurred while submitting quiz' });
        }
    });
